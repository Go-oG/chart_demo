...
List<dynamic> loadData1({int? limit, num maxRadius = 32}) {
  List<GraphData> nodes = [];
  List<EdgeData> links = [];
  List<dynamic> nl = _data1['nodes'] ?? [];
  List<dynamic> ll = _data1['links'] ?? [];
  Map<String, GraphData> nodeMap = {};
  ...
  for (var ele in nl) {
    ...
    String id = ele['id'] ?? '';
    num group = ele['group'] ?? 0;
    var node = GraphItemData(name: "$group", id: id, groupId: group.toInt().toString());
    double r = random.nextInt((maxRadius - 8).toInt()) + 8;
    node.width = r * 2;
    node.height = r * 2;
    node.weight = r * 2 / 3;
    var nd = GraphData(node);
    nodeMap[id] = nd;
    nodes.add(nd);
  }
  var node = nodes.first;
  node.width = maxRadius.toDouble();
  node.height = maxRadius.toDouble();
  for (var ele in ll) {
    String source = ele['source'] ?? '';
    String target = ele['target'] ?? '';
    num value = ele['value'] ?? 0;
    var sn = nodeMap[source];
    var tn = nodeMap[target];
    if (sn == null || tn == null) {
      continue;
    }

    EdgeItemData link = EdgeItemData(sn, tn);
    link.weight = value;
    links.add(EdgeData(link, 0));
  }
  return [nodes, links];
}

List<dynamic> loadData2() {
  List<GraphData> nodes = [];
  List<EdgeData> links = [];
  List<dynamic> nl = _data2['nodes'] ?? [];
  List<dynamic> ll = _data2['links'] ?? [];
  Map<String, GraphData> nodeMap = {};

  for (var ele in nl) {
    String id = ele['id'] ?? '';
    String group = ele['group'] ?? '';
    var itemData = GraphItemData(name: group.toText(), id: id);
    num r = ele['radius'] ?? 2;
    num w = ele['citing_patents_count'] ?? 0;
    itemData.width = r * 2;
    itemData.height = r * 2;
    itemData.weight = w.toDouble();
    var node = GraphData(itemData);
    nodeMap[id] = node;
    nodes.add(node);
  }
  for (var ele in ll) {
...
var _data2 = {
  "nodes": [
    ...
    {"id": "Catalysis by the nucleolytic ribozymes", "group": "Cited Works", "radius": 5, "citing_patents_count": 5},
    {
